...
func (mp *cBiRRTMotionPlanner) rrtBackgroundRunner(
	...
	if err != nil {
		rrt.solutionChan <- &rrtPlan{err: err}
		return
	}
	...
	for i := 0; i < mp.planOpts.PlanIter; i++ {
		...
		case <-ctx.Done():
			...
			rrt.solutionChan <- &rrtPlan{err: fmt.Errorf("cbirrt timeout %w", ctx.Err()), maps: rrt.maps}
			return
		default:
		}
		...
		if reachedDelta <= mp.planOpts.JointSolveDist {
			...
			cancel()
			path := rrt.maps.extractPath(&nodePair{map1reached, map2reached}, true)
			rrt.solutionChan <- &rrtPlan{steps: path, maps: rrt.maps}
			return
		}
	...
	rrt.solutionChan <- &rrtPlan{err: errPlannerFailed, maps: rrt.maps}
}
